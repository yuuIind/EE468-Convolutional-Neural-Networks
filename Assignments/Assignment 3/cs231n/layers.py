...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    X = np.reshape(x, (N, -1)) # X.shape = (N, D)
    out = np.dot(X, w) + b
...
def affine_backward(dout, cache):
    ...
    X = np.reshape(x, (x.shape[0], -1)) # X.shape = (N, D)
...
def softmax_loss(x, y):
    ...
    # substract max to prevent numerical instability when computing the exponential
    ...
    dx = probs.copy()
    # the true class is subtracted from the corresponding row of dx
    dx[np.arange(N), y] -= 1
    dx /= N
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # Store variables in cache for backward pass
